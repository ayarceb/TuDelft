...
L={'Suave','Normal','Jodido','Puteado','5','6','7','8','2','2','2','2','2','2','2','2','2','2'};
 ...
 for j=1:n
    if (0+(Particiones)*(j-1)<=max(Station(i),Station(i+1)))&&(max(Station(i),Station(i+1))<Minimo_Station+Particiones*(j))
        ...
        delete(h1(i-ventana))
            ...
            delete(h2)
            h2=plot(dates(i+1),Station(i+1),'Marker','o','MarkerSize',20,'Color','k');
            h1(i)=line([dates(i) dates(i+1)],[Station(i) Station(i+1)],'Color', A{j},'Marker','*','MarkerSize',10,'LineWidth',4);legend(L{j}),ylim([0 350]);
      dynamicDateTicks
%            axes('pos',[.1 .6 .5 .3])
